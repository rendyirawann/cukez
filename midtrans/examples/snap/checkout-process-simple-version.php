...
$id = $_GET["id"];
$query = $conn->query("SELECT * FROM riwayat WHERE id = '$id'")->fetch_object();
...
$item_details = array (
    array(
        ...
        'name' => "Cuci Mobil :( Tipe Mobil ".$query->tipe_mobil.' )'
    ),
  );
// Optional
    ...
    ?>  
      ...
      <?php  }else{ // if user is not logged in then display these buttons?>
        ...
        <a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#collapseBootstrap"
          aria-expanded="true" aria-controls="collapseBootstrap">
          <i class="fas fa-user"></i>
          <span>Registry Account(Not 100% Implemented)</span>
        </a>
        <div id="collapseBootstrap" class="collapse" aria-labelledby="headingBootstrap" data-parent="#accordionSidebar">
          <div class="bg-white py-2 collapse-inner rounded mt-1">
            <h6 class="collapse-header">MANAGE YOUR ACCOUNT</h6>  
            <a class="collapse-item" href="../../../admin.php?p=setting-account">Seeting Your Account</a>
          </div>
        </div>
      </li>
